...
export const CanvasConfigSchema = z.object({
  apiUrl: z.string().url('Canvas API URL must be a valid URL'),
  apiToken: z.string().min(1, 'Canvas API token is required'),
  apiTimeout: z.number().positive().default(30),
...
export class ConfigHandler {
  ...
  loadConfig(): CanvasConfig {
    try {
      ...
      this.config = CanvasConfigSchema.parse(rawConfig);
      return this.config;
    } catch (error) {
